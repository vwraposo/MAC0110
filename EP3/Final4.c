...
/* Funçao que verifica se uma palavra, contida no vetor palavra e de tamanho *j,
 está no dicionário */
 int dicionario ( char palavra[MAX], int *j) {
	char dic[MAX];
	/* vetor dic ira conter palavras do dicionário */
	int achei=0;
	/* achei é uma variavel que receberá o valor 1 quando a palavra buscada 
	for encontrada */
	int end=0;
	...
	/* O valor do tamanho do vetor é zerado, para o mesmo vetor ser 
	reutilizado na main */
	*j=0;
	fclose(entrada);
		...
		/* Decriptando com esse K */
			...
			for (i=0;i<n && end==0;i++) {
				if(plaintext[i]!='_' && plaintext[i]!='.') {
					...
					j++;					
					/* Caso nao haja um ponto ou espaço no 
					fim da palavra */
					if(i==(n-1))
						if (dicionario(palavra,&j)==0)
							end=1;
				}
				else if (j!=0){
