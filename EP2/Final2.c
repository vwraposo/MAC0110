...
int main () {
	...
	/* auxiliares para usar no vetor*/
				/*Nv3*/
	int k=MAX-1;
	/* k será dinamico, visando a melhor adaptaçã do computador para as 
	mudanças do padrão do usuário */
	int k2=5;
	/* k2 é uma variável para dinamizar a previsão da jogada do usuário 
	segundos os estudos encontrados nas fontes dadas pelo professor */ 
	int venceu;
		...
		while (usuario!=0) {		
			...
			if (computador==v[i])  {
				...
				venceu=2;
			 	...
			 	pode levar em conta mais rodadas */
			 	if (k<MAX-1) k++;
			 	if (k2<5) k2++;
			}
			else {
